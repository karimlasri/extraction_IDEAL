...
ANNOT_FIELDS = ['descriptive name of intervention', 'Outcome name used in the paper', 'details of the intervention']
    ...
    for f in os.listdir(annotations_folder):
        try:
            if f.endswith('.tsv') or f.endswith('.csv'):
                ...
                annotations[f.split('.')[0].split(' - ')[1]] = pd.read_csv(file_path, sep = sep)
        except Exception:
            pass
    return annotations
...
def get_annot_interv(annotations_df):
    ground_truth_col = [c for c in annotations_df.columns.values if c.startswith('Ground_Truth')][0]
    annots = {field:annotations_df[annotations_df['Field Name'].str.startswith(field)][ground_truth_col].tolist() for field in ANNOT_FIELDS}
...
def evaluate_io_for_sr(sr_annotations, sr_io_mentions, eval_type='em', model_type='google'):
	...
	for i, field in enumerate(ANNOT_FIELDS):
		for annot in annots[field]:
			extractions = [e for e in sr_io_mentions[EXTRACTION_FIELDS[i]].dropna().unique().tolist() if e!='xx']
			total[i]+=1
			sr_eval[i]+=look_for_io_annotation(annot, extractions, eval_type, model_type=model_type)
...
def evaluate_io(io_dfs, annotations, print_b=False, eval_type='em', model_type='google'):
    ...
    all_eval = {k:[] for k in ['pdf']+['{}_{}'.format(eval_type, f) for f in EXTRACTION_FIELDS]}
    for pdf, pdf_io_mentions in io_dfs.items():
        try:
            print('Evaluating extraction for {}'.format(pdf))
            pdf_eval, total = evaluate_io_for_sr(annotations[pdf], pdf_io_mentions, eval_type, model_type)
...
if __name__=='__main__':
    ...
    parser.add_argument("--separator", type=str, help="Separator used to parse annotation files.")
    ...
    all_scores = evaluate_io(io_dfs_tables, annotations, print_b=False, eval_type=eval_type, model_type='google')
    
    pd.DataFrame(all_scores).to_csv(f'src/evaluation/scores/io_eval_{eval_type}_{batch}.csv')